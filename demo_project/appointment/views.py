"""
预定系统-视图逻辑定义部分
分别包括：信息显示view、预订view以及查询取消view
--------------------------------
"""
from consume.models import consume_info
from personal_info.models import personal_info
from django.shortcuts import render, redirect
from appointment.models import Person, Tickets
from django.views import View
from .form import TicketForm, PersonForm
    ...
    if request.method == 'POST':
        ...
        if ticket_form.is_valid():
            ...
            if person.ticket_name == ticket.num:
                ...
                if time_day > 1:
                    message = '只能预定今天和明天的座位！'
                else:
                    if time_day == 0 and now_time.hour >= 23:
                        message = '当天超过晚上11点不可以进行预定'
                    else:
                        if ticket.seats >= 1:
                            message = '预订成功！'
                            ticket.seats -= 1
                            ...
                            person.save()
                            consume_info.objects.create(name=person.name, detail="预定座位",
                                                        money=person.ticket_money)
                            personal_new = personal_info.objects.get(id=1)
                            personal_new.money = personal_new.money - person.ticket_money
                            personal_new.save()
                        else:
                            message = '该车次暂无座位！'
                            person.delete()
